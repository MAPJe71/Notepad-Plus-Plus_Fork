...
static inline bool isWhiteSpace2( unsigned char  ch
                                , int &          nlCount
                                , unsigned char  excludeNewLine = 0
                                , unsigned char  chNext         = 0
                                )
{
    // multi-part keywords come in two flavors:
    // 1. "else if" (internally mapped to "else\vif") where '\v' can be 
    //          replaced by spaces, tabs and new lines
    // 2. 'else if" (internally mapped to "else\bif") where '\b' can be 
    //          replaced by spaces, tabs but not new lines
...
static bool isInListForward2( vvstring *      fwEndVectors[]
                            , int             totalVectors
                            , StyleContext &  sc
                            , bool            ignoreCase
                            , int             forward
                            )
{
    ...
    for ( int i = 0; i < totalVectors; ++i )
    {
        if ( fwEndVectors[i] && !fwEndVectors[i]->empty() )
        {
            ...
            for ( iter1 = fwEndVectors[i]->begin(); 
                  iter1 != fwEndVectors[i]->end(); 
                  ++iter1 
                )
            {
                iter2 = iter1->begin();
                for ( ; iter2 != iter1->end(); ++iter2 )
                {
                    ...
                    for ( ; ; ++iter3 )
                    {
                        a = ignoreCase 
                          ...
                          : *iter3;
                        b = ignoreCase 
                          ? toupper( sc.GetRelative( forward + index++ ) )
                          : sc.GetRelative( forward + index++ );
                        if (a != b)
                            break;
                        if ( iter3 != iter2->end() )
                            return true;
...
static bool isInListForward3( vector<string> *  tokens
                            , StyleContext &    sc
                            , bool              ignoreCase
                            , int               offset
                            , int &             moveForward
                            )
{
    ...
    for ( iter1 = tokens->begin(); iter1 != tokens->end(); ++iter1 )
    {
        ...
        {
            a = static_cast<unsigned char>( ignoreCase
                                          ? toupper( *iter2 )
                                          : *iter2
                                          );
            b = static_cast<unsigned char>( ignoreCase
                                          ? toupper( sc.GetRelative( offset + indexb++ ) )
                                          : sc.GetRelative( offset + indexb++ )
                                          );
            if ( a != b )
            {
                isFound = false;
...
static bool IsNumber(StyleContext &     sc
                    , vector<string> *  numberTokens[]
                    , vvstring *        fwEndVectors[]
                    , bool              ignoreCase
                    , int               decSeparator
                    , int &             moveForward 
                    )
{
    ...
    else
    {
        ...
        if (hasPrefix2 == false)
        {
            ...
            {
                if ( ignoreCase 
                   ? sc.MatchIgnoreCase2( iter->c_str() ) 
                   : sc.Match( iter->c_str() )
                   )
                    break;
            }
...
static bool isInListForward( vvstring &      openVector
                           , StyleContext &  sc
                           , bool            ignoreCase
                           , int &           openIndex
                           , int &           skipForward
...
static bool isInListBackward( WordList &      list
                            , StyleContext &  sc
                            , bool            specialMode
                            , bool            ignoreCase
                            , int &           moveForward
...
static void setBackwards( WordList *      kwLists[]
                        , StyleContext &  sc
                        , bool            prefixes[]
                        , bool            ignoreCase
                        , int             nestedKey
...
static bool isInListNested( int                      nestedKey
...
static void readLastNested( vector<nestedInfo> &  lastNestedGroup
                          , int &                 newState
                          , int &                 openIndex
                          )
{
...
static void ColouriseUserDoc( unsigned int  startPos
                            , int           length
                            , int           initStyle
                            , WordList *    kwLists[]
                            , Accessor &    styler
...
static void FoldUserDoc( unsigned int   /* startPos  */
                       , int            /* length    */
                       , int            /* initStyle */
                       , WordList *     /*           */[]
                       , Accessor &     /* styler    */
