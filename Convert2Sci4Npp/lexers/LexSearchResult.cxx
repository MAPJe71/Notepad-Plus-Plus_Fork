...
static enum {
      searchHeaderLevel = SC_FOLDLEVELBASE + 1
    , fileHeaderLevel
    , resultLevel
    };
...
static void ColouriseSearchResultDoc( unsigned int  startPos
                                    , int           length
                                    , int           /**/
                                    ...
                                    )
{
    ...
    for (unsigned int i = startPos; i < startPos + length; i++)
    {
        ...
        if (AtEOL(styler, i) || (linePos >= sizeof(lineBuffer) - 1))
        {
            ...
            ColouriseSearchResultLine( pMarkings
                                     , lineBuffer
                                     , linePos
                                     , startLine
                                     , i
                                     , styler
                                     , styler.GetLine(startLine)
                                     );
            linePos = 0;
            startLine = i + 1;
...
static void FoldSearchResultDoc( unsigned int  startPos
                               , int           length
                               , int           /**/
                               , WordList *    /**/[]
                               , Accessor &    styler
                               )
{
    ...
    for (unsigned int i = startPos; i < endPos; i++)
    {
        ...
        {
            lev = headerPoint
                ? SC_FOLDLEVELHEADERFLAG + headerPoint
                : resultLevel;
            headerPoint = 0;
...
LexerModule lmSearchResult( SCLEX_SEARCHRESULT
                          , ColouriseSearchResultDoc
                          , "searchResult"
                          , FoldSearchResultDoc
                          , emptyWordListDesc
                          );
